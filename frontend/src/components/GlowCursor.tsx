...
export const GlowCursor: React.FC<GlowCursorProps> = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener('pointermove', updatePointer as any, { passive: true });
    container.addEventListener('pointerenter', updatePointer as any);
    ...
    return () => {
      ...
      window.removeEventListener('pointermove', updatePointer as any);
      container.removeEventListener('pointerenter', updatePointer as any);
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        className="pointer-events-none absolute inset-0 block h-full w-full select-none z-10"
        style={{ mixBlendMode: blendMode }}
        aria-hidden="true"
      />
