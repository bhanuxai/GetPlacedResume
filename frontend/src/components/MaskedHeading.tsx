...
export const MaskedHeading: React.FC<MaskedHeadingProps> = ({
  ...
  const TagAny = (tag || 'h2') as any;

  return (
    <TagAny
      ref={rootRef}
      className={`relative w-full m-0 p-0 antialiased [text-wrap:balance] select-none ${className}`.trim()}
      style={{
        textAlign: align,
        fontWeight: weight,
      ...
      {...rest}
    >
      <span ref={measureRef} className="text-transparent pointer-events-none select-none">
        {words.map((word, i) => (
          <span
            key={`${word}-${i}`}
